...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < epsilon_set_size; ++i)
	{
		...
		runs_global_num_final[i] = new int[timesOfRun];
	...
	double **Sum_Final_Size = new double *[funNum];
	for (i = 0; i < funNum; i++)
		Sum_Final_Size[i] = new double[epsilon_set_size];
	...
	double *minFitness_Times = new double[funNum];
	...
	for (fun = 0; fun < funNum; fun++)
	{
		...
		double **population_new = new double*[population_size];
		double ** species = new double*[population_size];

		for (i = 0; i < population_size; ++i)
		{
			population[i] = new double[dim];
			population_new[i] = new double[dim];
			species[i] = new double[dim];
		}
		double *species_results = new double[population_size];
		double *tmp_Chromosome = new double[dim];
		double *results = new double[population_size];
		double *individual_best = new double[dim];
		double *individual_trial = new double[dim];
		double individual_best_result = 0;
		...
		Sum_Archive[fun] = 0;
		memset(Sum_Final_Size[fun], 0, sizeof(double) * epsilon_set_size);
		No_Leaf_Initialization = 0;
		Better_Times[fun] = 0;

		Middle_Times[fun] = 0;
		Worse_Times[fun] = 0;
		minFitness_Times[fun] = 0;
		// parameter setting
		...
		for (run = 0; run < timesOfRun; run++)
		{
			...
			for (j = 0; j < dim; ++j)
			{
				...
				for (i = 0; i < population_size; ++i)
				{
					population[i][j] = population_new[i][j] = random_real_num_x();
				}
			}

			Popupation_Fitness(population, population_size, Fes, results, pFunc);
			
			//Build the never visit
			...
			{
				if (results[best_index] < results[i])
					best_index = i;
			}
			individual_best_result = results[best_index];
			memcpy(individual_best, population[best_index], sizeof(double) * dim);
			while (Fes < MAX_FES)
			{
				Evolve_NrNSDE_V5_random_centroid(population, population_new, results, individual_trial,
					individual_best, individual_best_result, UBound, LBound,
					population_size, parameter_m, F, CR, dim, Fes, pFunc,
					generator, species, species_results, archive, fitnessArchive, minFitness,
					tmp_Chromosome, Leaf_Vector, &cur_GA, axis_range, inner_radius);
				VisitStrategy_V9(population, population_new, results, individual_trial,
					individual_best, individual_best_result, UBound, LBound,
					population_size, parameter_m, F, CR, dim, Fes, pFunc,
					generator, species, species_results, archive, fitnessArchive, minFitness,
					tmp_Chromosome, Leaf_Vector, &cur_GA, axis_range, inner_radius, inter_radius);
			} //while( Fes < MAX_FES )
	
			char resultFile[200];
			sprintf(resultFile, "Results/DATA_PNF_DE/PNF_DE_random_Axis_%d_Rinner_%f_Rinter_%f_NP_%d_m_%d.txt", axis_range, DIST_INNER_CLUSTER, DIST_INTER_CLUSTER, parameter_NP, parameter_m);

			vector<double> indiv(dim);
			for (i = 0; i < population_size; i++)
			{
				for (j = 0; j < dim; j++)
					indiv[j] = population[i][j];
				archive.push_back(indiv);
				fitnessArchive.push_back(results[i]);
			}
			...
			for (int ep = 0; ep < epsilon_set_size; ++ep)
			{
				...
				Sum_Final_Size[fun][ep] += finalSize;
			...
			for (i = 0; i < epsilon_set_size; ++i)
			{
				...
				if (runs_global_num[i][run] == global_optima_num)
					SR[fun][i] ++;
		...
		snprintf(NP_counter, 10, "%d", parameter_NP);
		snprintf(m_counter, 10, "%d", parameter_m);
		...
		for (i = 0; i < epsilon_set_size; ++i)
		{
			...
			Epsilon_Files[i] = "./Results/DATA_PNF_DE/" + string(epsilon_counter) + "/" + "Final_NumOptima_Epsilon_" + string(epsilon_counter)
				+ "_Func_" + string(fun_name) + "_Axis_" + string(axis_num) + "_Rinner_" + string(radius_num_inner) + "_Rinter_" + string(radius_num_inter) +
				"_NP_" + string(NP_counter) + "_m_" +
				string(m_counter) + ".txt";
		}
		for (i = 0; i < epsilon_set_size; ++i)
		{
		...
		minFitness_Times[fun] /= (double)timesOfRun;
		...
		Sum_Archive[fun] /= (double)timesOfRun;
		for (int ep = 0; ep < epsilon_set_size; ep++)
			Sum_Final_Size[fun][ep] /= (double)timesOfRun;
	...
	for (i = 0; i < funNum; i++)
	{
		...
		cout << "Mean Final Archive 1.E-1:\t" << Sum_Final_Size[i][0] << endl;
		cout << "Mean Final Archive 1.E-2:\t" << Sum_Final_Size[i][1] << endl;
		cout << "Mean Final Archive 1.E-3:\t" << Sum_Final_Size[i][2] << endl;
		cout << "Mean Final Archive 1.E-4:\t" << Sum_Final_Size[i][3] << endl;
		cout << "Mean Final Archive 1.E-5:\t" << Sum_Final_Size[i][4] << endl;
		...
		outFile << Sum_Final_Size[i][0] << endl;
		outFile << Sum_Final_Size[i][1] << endl;
		outFile << Sum_Final_Size[i][2] << endl;
		outFile << Sum_Final_Size[i][3] << endl;
		outFile << Sum_Final_Size[i][4] << endl;
	...
	for (i = 0; i < epsilon_set_size; ++i)
	{
		...
		delete[]runs_global_num_final[i];
	...
	delete[]Middle_Times;
	delete[]Worse_Times;
	delete[]minFitness_Times;
	for (i = 0; i < funNum; i++)
	{
		delete[]Sum_Final_Size[i];
	}
	delete[]Sum_Final_Size;
